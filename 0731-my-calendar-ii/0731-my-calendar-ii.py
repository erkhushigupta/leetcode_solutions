...
class MyCalendarTwo:
    def __init__(self):
        # Initialize a SortedDict to keep track of the booking times.
    ...
    def book(self, start: int, end: int) -> bool:
        ...
        for count in self.booking_counts.values():
            ...
            # overlapping events at current time.
            running_sum += count

            # If there are more than 2 simultaneous events, it's a conflict.
            if running_sum > 2:
                # The booking is invalid, so we revert the increment and decrement
                # for the start and end time of the new event.
                self.booking_counts[start] -= 1
                self.booking_counts[end] += 1

                # Return False indicating booking was unsuccessful.
                return False
